...
__all__ = ['gsnetNet', 'get_gsnetnet']


class gsnetNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(gsnetNet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = gsnetNetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class gsnetNetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(gsnetNetHead, self).__init__()
        self.se_loss = se_loss
        inter_channels = in_channels // 4

        self.aa_gsnet = gsnet_Module(in_channels, inter_channels, atrous_rates, norm_layer, up_kwargs)
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_gsnet(x)
...
class gsnet_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        super(gsnet_Module, self).__init__()
...
def get_gsnetnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = gsnetNet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
