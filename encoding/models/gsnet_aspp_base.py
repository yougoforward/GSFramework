...
__all__ = ['gsnet_aspp_baseNet', 'get_gsnet_aspp_basenet']


class gsnet_aspp_baseNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(gsnet_aspp_baseNet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = gsnet_aspp_baseNetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class gsnet_aspp_baseNetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(gsnet_aspp_baseNetHead, self).__init__()
        self.se_loss = se_loss
        inter_channels = in_channels // 4

        self.aa_gsnet_aspp_base = gsnet_aspp_base_Module(in_channels, inter_channels, atrous_rates, norm_layer, up_kwargs)
        self.conv8 = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(inter_channels, out_channels, 1))
        if self.se_loss:
            self.selayer = nn.Linear(inter_channels, out_channels)

    def forward(self, x):
        feat_sum, gap_feat = self.aa_gsnet_aspp_base(x)
...
def gsnet_aspp_baseConv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class gsnet_aspp_basePooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(gsnet_aspp_basePooling, self).__init__()
...
class gsnet_aspp_base_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        super(gsnet_aspp_base_Module, self).__init__()
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = gsnet_aspp_baseConv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = gsnet_aspp_baseConv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = gsnet_aspp_baseConv(in_channels, out_channels, rate3, norm_layer)
...
def get_gsnet_aspp_basenet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = gsnet_aspp_baseNet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
