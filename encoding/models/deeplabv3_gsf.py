...
__all__ = ['deeplabv3_gsf', 'get_deeplabv3_gsf']

class deeplabv3_gsf(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(deeplabv3_gsf, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = deeplabv3_gsfHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
def get_deeplabv3_gsf(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = deeplabv3_gsf(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
