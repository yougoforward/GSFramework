...
class psp_gsf(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psp_gsf, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = psp_gsfHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class psp_gsfHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psp_gsfHead, self).__init__()
...
def get_psp_gsf(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = psp_gsf(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('psp_gsf_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model

def get_psp_gsf_resnet50_ade(pretrained=False, root='~/.encoding/models', **kwargs):
    r"""psp_gsf model from the paper `"Context Encoding for Semantic Segmentation"
    ...
    Examples
    --------
    >>> model = get_psp_gsf_resnet50_ade(pretrained=True)
    >>> print(model)
    """
    return get_psp_gsf('ade20k', 'resnet50', pretrained, root=root, **kwargs)
